export const requirementFields = [
  { key: 'goal', label: 'Goal', description: 'What is the desired outcome?' },
  {
    ...
    description: 'Why is this change needed and what is the business driver?',
  },
  ...
  },
  {
    ...
    description: 'Quality attributes such as performance, security, compliance.',
  },
  {
    key: 'risks_and_dependencies',
    label: 'Risks and Dependencies',
    description: 'Known risks, blockers, or external dependencies.',
  },
] as const;
...
export function createEmptyRequirementsContext(): RequirementsContext {
  return Object.fromEntries(
    requirementFields.map((field) => [field.key, null])
  ) as RequirementsContext;
}

export function allFieldsFilled(context: RequirementsContext): boolean {
  return requirementFields.every((field) => {
...
## Open Questions
`,
  fields: requirementFields.map((field) => ({
    key: field.key,
    label: field.label,
